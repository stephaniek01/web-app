...
const LoginPage = () => {
    ...
    const handleSubmit = (values, {setSubmitting}) => {
        setLoading(true);
        if (values.email!="admin@admin.com" || values.password!="12345678") {
            console.log("error")
            setError("Login credentials not matched!");
        } else {
            localStorage.setItem("authToken", "jskdhbsajhdsajhdjashbbeuy4bebb.sfdhijhniunhie.fedfedibnihub.fdeeedsdsdsdjhbniuryuiir")
            setTimeout(() => {
                navigate('/');
            }, 500);
        }
